...
export default async function SubportfolioPage({ params }: PageProps<"/[subportfolio]">) {
  ...
  const sections = subportfolio.categories
    .map((categorySlug): { category: CategoryMeta; projects: Project[] } | null => {
      const category = getCategory(categorySlug);
      if (!category) return null;
      const projects = allProjects.filter(
        (p) => p.category === categorySlug && subportfolio.projects.includes(p.slug)
      );
